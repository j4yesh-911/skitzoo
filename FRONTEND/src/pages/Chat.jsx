...
const IconMic = ({ active }) => (
  <svg
    ...
    viewBox="0 0 24 24"
    fill="none"
    stroke={active ? "#f87171" : "currentColor"}
    strokeWidth="2.2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
...
const WAVE = [
  4, 8, 14, 6, 18, 10, 5, 16, 9, 20, 7, 13, 4, 17, 8, 12, 5, 15, 9, 6, 11, 7,
];
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const statusHandler = ({ chatId: sc, messageIds, messageId, status }) => {
      ...
      setMessages((p) =>
        p.map((m) => (ids.includes(m._id) ? { ...m, status } : m)),
      );
    };
  ...
  useEffect(() => {
    fetch(`https://skitzoo-1.onrender.com/api/messages/${chatId}`, {
      headers: {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("https://skitzoo-1.onrender.com/api/chats", {
        ...
        setOtherUserName(
          other?.name || other?.username || other?.displayName || "Unknown",
        );
      } catch (err) {
        console.error(err);
      }
    ...
    (async () => {
      try {
        const res = await fetch(
          `https://skitzoo-1.onrender.com/api/users/${otherUserId}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    API.get(`/reports/check/${otherUserId}`)
      .then((r) => {
        if (r.data?.blocked) setInteractionBlocked(true);
      })
      .catch(() => {});
  }, [otherUserId]);
  ...
  const sendVoiceMessage = async (blob) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://skitzoo-1.onrender.com/api/messages/upload",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem("token")}`,
  ...
  const Tick = ({ status }) => {
    ...
    if (status === "seen")
      return <span style={{ ...s, color: "#fbbf24" }}>✓✓</span>;
    if (status === "delivered")
      return <span style={{ ...s, color: "rgba(255,255,255,0.3)" }}>✓✓</span>;
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                "radial-gradient(ellipse 90% 55% at 50% -5%, #12101e 0%, #0a0a0f 60%)",
            }}
          />
          {[
            {
          ...
          <div
            style={{
              ...
              opacity: 0.018,
              backgroundImage:
                "linear-gradient(rgba(124,106,255,1) 1px,transparent 1px),linear-gradient(90deg,rgba(124,106,255,1) 1px,transparent 1px)",
        ...
        >
          <div
            style={{
              ...
              height: 1,
              background:
                "linear-gradient(90deg,transparent,rgba(124,106,255,0.45),rgba(167,139,250,0.3),transparent)",
            }}
          />
          <div
            style={{
          ...
          >
            ...
            <button
              className="ch-icon-btn"
              onClick={() => {
                if (!interactionBlocked) setVideoOpen(true);
              }}
                  ...
                  >
                    ...
                    >
                      <div
                        ...
                        style={{
                          padding:
                            m.type === "voice" ? "10px 14px" : "10px 15px",
                          borderRadius: isMine
                            ? "18px 18px 4px 18px"
                            : "18px 18px 18px 4px",
                          ...(isMine
                        ...
                        {m.type === "voice" ? (
                          ...
                          >
                            <motion.button
                              ...
                              whileTap={{ scale: 0.9 }}
                              onClick={() =>
                                playVoiceMessage(m._id, m.audioUrl)
                              }
                              style={{
                                flexShrink: 0,
                                width: 32,
                                ...
                                justifyContent: "center",
                                cursor: "pointer",
                                color: isMine ? "#fff" : "var(--ch-accent2)",
                              }}
                            >
        ...
        >
          <div
            style={{
              ...
              height: 1,
              background:
                "linear-gradient(90deg,transparent,rgba(124,106,255,0.22),transparent)",
            }}
          />
          {interactionBlocked && (
            <div
                ...
                >
                  <div
                    style={{
                      ...
                      animation: "ch-recpulse 1.1s ease infinite",
                    }}
                  />
                  <span
                    style={{
