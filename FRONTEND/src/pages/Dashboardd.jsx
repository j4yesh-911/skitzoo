...
function SidebarContent({ user, posts, swappers }) {
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            <img
              src={getAvatar(user.profilePic, user.name)}
              style={{
                width: 44,
                height: 44,
                borderRadius: "50%",
              ...
              alt=""
              onError={(e) => (e.target.src = getAvatar(null, user.name))}
            />
            <span
              style={{
                position: "absolute",
      ...
      {swappers.length === 0 && (
        <p style={{ fontSize: 12, color: "#334155" }}>No swappers yet 🤝</p>
      )}
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {swappers.map((s) => (
          <div key={s._id} className="swapper-row">
            <img
              src={getAvatar(s.profilePic, s.name)}
              style={{
                width: 30,
                height: 30,
                borderRadius: "50%",
              ...
              alt=""
              onError={(e) => (e.target.src = getAvatar(null, s.name))}
            />
            <span
              style={{
                fontSize: 13,
...
function RightPanelContent({ posts, swappers, setShowTpl, setActiveTpl }) {
  ...
  const thisWeekPosts = posts.filter(
    (p) => new Date() - new Date(p.createdAt) < 7 * 24 * 60 * 60 * 1000,
  ).length;
  const activityMap = {};
  posts.forEach((p) => {
  ...
  const days = Array.from({ length: 56 }, (_, i) => {
    ...
    return { date: d, count: activityMap[d.toDateString()] || 0 };
  });
  const maxActivity = Math.max(...days.map((d) => d.count), 1);
  const tips = [
    "🎯 Try a Skill Swap to grow faster",
    "📅 Consistent posts build trust",
    "💬 Sessions with Q&A get 2x engagement",
    "🌱 Share progress, not just results",
    "🔗 Always include a meeting link",
  ];
    ...
    <>
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {[
            {
              icon: <TrendingUp size={14} color="#818cf8" />,
            ...
            {
              icon: <Flame size={14} color="#4ade80" />,
              bg: "rgba(34,197,94,0.12)",
              label: "This week",
              val: `${thisWeekPosts} posts`,
            },
            {
              icon: <Users size={14} color="#fbbf24" />,
          ...
          >
            {days.map((d, i) => (
              <div
                ...
                title={`${d.date.toDateString()}: ${d.count} post(s)`}
                style={{
                  background:
                    d.count === 0
                      ? "rgba(255,255,255,0.04)"
                      : `rgba(99,102,241,${0.2 + (d.count / maxActivity) * 0.75})`,
                  borderRadius: 3,
                }}
              />
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {TEMPLATES.map((tpl) => (
            ...
            >
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: tpl.color }}>
                {tpl.label}
              </span>
            </button>
          ))}
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ padding: "0 20px" }}>
                      <textarea
                        ...
                        placeholder="Share a skill session, meeting link, or learning update…"
                      />
                    </div>
                  ...
                  {feedPosts.length === 0 && (
                    ...
                    >
                      ...
                      <p style={{ fontSize: 13 }}>
                        Create your first post or add swappers to see their
                        updates
                      </p>
                    </div>
                  )}
        ...
        {deleteConfirm && (
          ...
          >
            ...
            >
              ...
              >
                This can't be undone. Your session post will be permanently
                removed.
              </p>
        ...
        {showTpl && (
          ...
          >
            ...
            >
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#475569", marginTop: 2 }}>
                    Pick a template, fill in details, post instantly
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div
                style={{ overflow: "auto", padding: "20px 24px 24px", flex: 1 }}
              >
                {!activeTpl ? (
                  <div
                    style={{
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        ...
                        padding: "12px 14px",
                        borderRadius: 12,
                        background: activeTpl.bg,
                        border: `1px solid ${activeTpl.border}`,
                      }}
                    >
                      ...
                      {[
                        ...
                        >
                          ...
                          <input
                            ...
                            placeholder={f.ph}
                            onFocus={(e) =>
                              (e.target.style.borderColor = activeTpl.color)
                            }
                            onBlur={(e) =>
                    ...
                    >
                      ...
                      >
                        {activeTpl.fill(tplData)}
                      </p>
                    </div>
                    <button
                      onClick={() => useTpl(activeTpl)}
                      style={{
                        width: "100%",
                        marginTop: 14,
                        padding: 13,
                        borderRadius: 13,
                        background: `linear-gradient(135deg,${activeTpl.color},${activeTpl.color}bb)`,
                        border: "none",
                        color: "white",
                        fontWeight: 700,
                        fontSize: 14,
